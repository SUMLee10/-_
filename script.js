...
function handleExplodedAction(row, col, game, cellEl, tableEl) {
...
function handleWin(row, col, game, cellEl, tableEl) {
    ...
    panelEl.innerHTML =`
        <h3>恭喜，您在${game.seconds}秒成功排除</h3>
    `;
...
function handleClearAction(row, col, game, cellEl, tableEl ) {
    let value = game.getCellValue(row, col);
    if (value === -1){
        handleExplodedAction(row, col, game, cellEl, tableEl);
        return;
    }
...
function renderWelcome() {
    ...
    gameEl.innerHTMLL = `
    <div class="welcome">
    <button id="level-0">初级</button>
    <button id="advance"> 高级</button>
    </div>
    `;

    let buttonEl = gameEl.querySelector("button#level-0");
    buttonEl.onclick = () => {
